...
public class ImagePickerDelegate
    ...
    private void launchTakeVideoWithCameraIntent() {
        ...
        if (this.methodCall != null && this.methodCall.argument("quality") != null) {
            int quality = this.methodCall.argument("quality");
    ...
    public boolean onActivityResult(int requestCode, int resultCode, Intent data) {
        switch (requestCode) {
            ...
            case REQUEST_CODE_TAKE_VIDEO_WITH_CAMERA:
                if (data != null) {
                    pendingCameraMediaUri = data.getData();
                    System.out.println("----返回视频--" + pendingCameraMediaUri.toString());
                } else {
                }
...
//                        @Override
//                        public void onPathReady(String path) {
//                            System.out.println("----解析的视频路径："+path);
//                            handleVideoResult(path);
//                        }
//                    });
            String path;
            if (pendingCameraMediaUri == null) {
                path = cache.retrievePendingCameraMediaUriPath();
            } else {
                path = pendingCameraMediaUri.getPath();
            }
            if (path != null) {
                System.out.println("----path:" + path);
                String[] param = path.split("/");
                if (param != null && param.length > 0) {
                    handleVideoResult(param[param.length - 1]);
                }
            }
            return;
        }
